...
MsgBox::MsgBox(QWidget *parent) : DropShadowWidget(parent) {
  ...
  main_layout->setContentsMargins(SHADOW_WIDTH, SHADOW_WIDTH, SHADOW_WIDTH,
                                  SHADOW_WIDTH);
  ...
  QObject::connect(close_button, SIGNAL(clicked()), this,
                   SLOT(cancelOperate()));
  QObject::connect(cancel_button, SIGNAL(clicked()), this,
                   SLOT(cancelOperate()));
...
void MsgBox::setInfo(QString title_info, QString info, QPixmap pixmap,
                     bool is_ok_hidden) {
...
void MsgBox::paintEvent(QPaintEvent *event) {
  ...
  painter.drawPixmap(
      ...
      QPixmap(skin_name));
  painter.drawRect(QRect(SHADOW_WIDTH, height, this->width() - 2 * SHADOW_WIDTH,
                         this->height() - height - SHADOW_WIDTH));
}

void MsgBox::okOperate() { this->accept(); }

void MsgBox::cancelOperate() { this->reject(); }
